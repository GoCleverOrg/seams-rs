# ============================================================================
# seams-rs workspace targets
#
# Mirrors the vmb-rs Makefile's dev-experience (fmt/clippy/deny/shear/taplo/
# typos, per-crate mutants, bench harness). Pure-stdlib — no native deps.
# ============================================================================
...
CRATES      := seams-rs-core seams-rs-fake seams-rs-std seams-rs

# Warm-path optimizations ported from mira's mutants-perf experiments
# (see docs/mutants-perf-learnings.md in the mira repo for rationale).
...
rust-mutants-crate:
	@test -n "$(CRATE)" || (echo "Usage: make rust-mutants-crate CRATE=seams-rs-core" && exit 1)
	@mkdir -p $(MUTANTS_OUT)
	@$(MUTANTS_TERM_ENV) $(RUST_ENV) cargo mutants --package $(CRATE) \
		--timeout=$(MUTANTS_TIMEOUT) --in-place \
		--output $(MUTANTS_OUT)/$(CRATE) \
...
rust-mutants-file:
	@test -n "$(FILE)" || (echo "Usage: make rust-mutants-file FILE=seams-rs-core/src/lib.rs" && exit 1)
